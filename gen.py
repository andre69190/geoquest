...
function mpCreate(){
  ...
  const code=mpCode();
  const channel=sb.channel("room_"+code,{config:{broadcast:{self:false}}});
...
function mpJoin(code){
  ...
  const channel=sb.channel("room_"+uc,{config:{broadcast:{self:false}}});
...
function renderMultiplayerLobby(){
  ...
  if(\!mp){
    ...
    return`<div class="scr">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:1.2rem">
        <button class="btn-cancel" onclick="S.mpModal=false;render()">←</button>
        <h2 style="font-size:1.3rem;font-weight:900;color:var(--text);margin:0">⚔️ Live 1vs1 Duell</h2>
      </div>
  ...
  if(mp.phase==="waiting"){
    return`<div class="scr" style="text-align:center">
      <button class="btn-cancel" style="float:left" onclick="mpLeave()">←</button>
  ...
  if(q.type==="flagsel"){
    ...
    if(q.type==="pop_compare"&&q.subj&&typeof q.subj==='object'){
      return`${topBar}
        ...
        ${sel?`<button class="btn-p" onclick="nextRound()">Weiter \u2192</button>`:""}`;
    }
