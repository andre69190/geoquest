...
const ADMIN_EMAIL   = "admin@geoquest.local";
...
function genCurrRealQ(){
  ...
  const dis=CURR_REAL.filter((_,i)=>i!==idx).sort(()=>Math.random()-.5).slice(0,3).map(x=>x.c+" ("+x.iso+")");
  const ans=cor.c+" ("+cor.iso+")";
...
function genPopCompareQ(){
  ...
  const ans=b.pop>a.pop?"Mehr \u2b06\ufe0f":"Weniger \u2b07\ufe0f";
  const wrong=b.pop>a.pop?"Weniger \u2b07\ufe0f":"Mehr \u2b06\ufe0f";
...
if(sbOK){try{sb=supabase.createClient(SUPABASE_URL,SUPABASE_ANON);initAuth();}catch(e){}}
...
async function loadProfile(){
  ...
  sbProfile=data;
  if(sbUser.email===ADMIN_EMAIL&&(\!sbProfile?.is_premium)){
    await sb.from("profiles").update({is_premium:true,geo_coins:999999}).eq("id",sbUser.id);
    if(sbProfile){sbProfile.is_premium=true;sbProfile.geo_coins=999999;}
  }
...
async function saveSession(mode,score,bs,correct){
  if(\!sb||\!sbUser)return;
  ...
  await sb.from("profiles").update({total_score:(sbProfile?.total_score||0)+score,games_played:(sbProfile?.games_played||0)+1}).eq("id",sbUser.id);
...
function answer(a){
  ...
  if(ok){const ns=S.st+1,t=tier(ns);const mm=S.diff==="hardcore"?3:S.diff==="survival"?2:1;pts=~~((BASE+S.tm*TB)*t.x*mm);S.sc+=pts;S.st=ns;S.bs=Math.max(S.bs,ns);S.correct++;soundCorrect();if(ns>=3)setTimeout(()=>soundStreak(ns),250);showPtsPopup(pts);if(navigator.vibrate)navigator.vibrate([50]);}
    ...
    if(S.diff==="survival"){
      ...
      S.ph="gameover";S.scoreSaved=false;soundOver();checkMastery();
      ...
      if(sbOK)saveSession(S.mode,S.sc,S.bs,S.correct).then(()=>{S.scoreSaved=true;render();});
...
function startGame(m){
  ...
  Object.assign(S,{sc:0,st:0,bs:0,rd:0,correct:0,lid:null,ph:"playing",mode:m||S.mode,
    scoreSaved:false,sessionAnswers:[],newStamps:[],isDailyRun:false,challengeStarted:false,
    half_removed:false,freezeActive:false,queueExtra:[],askedLids:new Set(),
    survivalBest:survBest});
  lq();
}
...
function startDailyChallenge(){
  ...
  Object.assign(S,{sc:0,st:0,bs:0,rd:0,correct:0,lid:null,ph:"playing",scoreSaved:false,sessionAnswers:[],newStamps:[],half_removed:false,freezeActive:false,queueExtra:[],askedLids:new Set()});
  lq();
}
...
function render(){
  ...
  if(S.ph==="gameover"){
    ...
    app.innerHTML=`<div class="scr">
      ...
      ${isGuest&&S.correct>0?`<div class="conv-modal-bg" onclick="if(event.target===this){S.convModal=false;render()}">
  ...
  if(q.type==="flagsel"){
    ...
    if(q.type==="pop_compare"&&q.subj&&typeof q.subj==='object'){
      return`${topBar}
        ...
        <div class="btns">${q.opts.map(o=>{let cls="btn-a";if(sel!==null){if(o===q.ans)cls+=" ok";else if(o===sel)cls+=" ng";else cls+=" dm";}return`<button class="${cls}" ${sel?"disabled":""} onclick="answer('${o.replace(/'/g,"\'")}')">${o}</button>`;}).join("")}</div>
        ${sel?`<div class="meta-line">${q.meta}</div>`:""}
        ${sel?`<button class="btn-p" onclick="lq()">Weiter \u2192</button>`:""}`;
    }
    const btns=q.opts.map(o=>{let cls="btn-a";const os=o.replace(/'/g,"\\'");if(sel\!==null){if(o===q.ans)cls+=" ok";else if(o===sel)cls+=" ng";else cls+=" dm";}const mk=sel?(o===q.ans?`<span>\u2713</span>`:o===sel?`<span>\u2717</span>`:""):"";return`<button class="${cls}" ${sel?"disabled":""} onclick="answer('${os}')">${o}${mk}</button>`;}).join("");
...
function renderLeaderboard(lbData,lbLoading,mode){
  ...
  return`<div style="display:flex;gap:5px;margin-bottom:.75rem;flex-wrap:wrap">${MODES.map(m=>`<button onclick="S.mode='${m.id}';showLeaderboard()" style="flex:1;min-width:36px;background:${mode===m.id?"#10b981":"var(--bg3)"};color:${mode===m.id?"#fff":"var(--text2)"};border:1px solid var(--border);border-radius:7px;padding:.3rem .2rem;font-size:.8rem;font-weight:700;cursor:pointer">${m.icon}</button>`).join("")}</div>
    ${lbData.map((r,i)=>{const rc=i===0?"gold":i===1?"silver":i===2?"bronze":"";const isMe=sbUser&&r.user_id===sbUser.id;const isPromo=i<5;const isRel=i>=n-5;let cls="lb-row";if(isMe)cls+=" me";else if(isPromo)cls+=" promo";else if(isRel)cls+=" relg";return`<div class="${cls}"><span class="lb-rank ${rc}">${r.rank}</span><span class="lb-name">${r.username||"Anonym"}${isMe?`<span style="color:#34d399;font-size:.7rem;margin-left:4px">Du</span>`:""}</span><span class="lb-score">${Number(r.best_score||0).toLocaleString()}</span></div>`;}).join("")}`;
}
...
/* CHALLENGE (Phase 16) */
const CHALLENGE=(()=>{try{const p=new URLSearchParams(location.search);const gq=p.get("gq");if(gq){const[s,o,m]=gq.split(":");return{seed:parseInt(s),oppScore:parseInt(o),mode:m||"city"};}}catch(e){}return null;})();
